...
{  
  ...
  bool network_joined = lmh_join_status_get() == LMH_SET;

  if(network_joined){
    // Every 20 sec. send frame
    if (currentMillis - previousMillis_send_frame >= interval_send_frame) {
      // save the last time you send_frame
      previousMillis_send_frame = currentMillis;
  ...
  // Every 1sec.
  if (currentMillis - previousMillis_lora >= interval_lora) {
...
static void lorawan_join_failed_handler(void)
{
  ...
  // re-start Join procedure
  Serial.println("Restarting Join procedure...");
  lmh_join();
}
...
void lorawan_rx_handler(lmh_app_data_t *app_data)
{
  ...
  if(!(strcmp((char*)app_data->buffer, "start_gps"))){
    Serial.println("Starting gps now");
    Serial1.begin(9600);
  } 
  else if(!(strcmp((char*)app_data->buffer, "end_gps"))){
    Serial.println("Ending gps now");
    Serial1.end();
  }
}
